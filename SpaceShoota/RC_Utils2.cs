...
namespace RC_Framework
{
  ...
  // ------------------------------- Util Class ------------------------------------------------------------------------
    ...
    public class Util
    {
        ...
        /// <summary>
        /// This will replace one colour with another in a 'Color' Format texture
...
// ********************** texture routines to help XNA/Monogame ***********************************************************
        /// <summary>
        /// Loads a texture from a file (usefull in debugging)
        /// </summary>
        /// <param name="gd"> a valid GraphicsDevice</param>
        /// <param name="fName"> The name of the file eg ,dir+"fred.png" </param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Returns a Rectangle that is the entire texture
        /// </summary>
        /// <param name=""></param>
        /// <returns></returns>
        public static Rectangle texRectangle(Texture2D tex)
        {
...
// ********************************* timing functions ****************************************
        ...
        // ---------------------- is Rectangle1 inside Rectange2 ----------------------------------------------------------------------------------

        public static bool rectangleInside(Rectangle rectangle1, Rectangle rectangle2)
        {
            if (!rectangle2.Contains(new Point(rectangle1.X, rectangle1.Y))) return false;
            if (!rectangle2.Contains(new Point(rectangle1.X + rectangle1.Width, rectangle1.Y))) return false;
            if (!rectangle2.Contains(new Point(rectangle1.X, rectangle1.Y + rectangle1.Height))) return false;
            if (!rectangle2.Contains(new Point(rectangle1.X + rectangle1.Width, rectangle1.Y + rectangle1.Height))) return false;
            return true;
        }
        ...
        public static void drawStringAtY(SpriteBatch sb, SpriteFont font, string s, int screenWidth, int y, Color col)
        {
            Vector2 size = font.MeasureString(s);
            Vector2 p = new Vector2(screenWidth / 2 - size.X / 2, y);
            sb.DrawString(font, s, p, col);
        }

        // ------------------ inner and outer rectangle mask ---------------------------------------------------------------------------------------

        public static void drawMask(SpriteBatch sb, Rectangle outerRectangle, Rectangle innerRectangle, Color maskColor)
        {

            int h = innerRectangle.Y - outerRectangle.Y;
            Rectangle r = new Rectangle(outerRectangle.X, outerRectangle.Y, outerRectangle.Width, h);
            sb.Draw(LineBatch._empty_texture, r, maskColor);

            int w = innerRectangle.X - outerRectangle.X;
            r = new Rectangle(outerRectangle.X, outerRectangle.Y + h, w, innerRectangle.Height);
            sb.Draw(LineBatch._empty_texture, r, maskColor);

            r = new Rectangle(innerRectangle.X + innerRectangle.Width, outerRectangle.Y + h, outerRectangle.Width - innerRectangle.Width - w, innerRectangle.Height);
            sb.Draw(LineBatch._empty_texture, r, maskColor);

            r = new Rectangle(outerRectangle.X, innerRectangle.X + innerRectangle.Height, outerRectangle.Width, outerRectangle.Height - innerRectangle.Height - innerRectangle.Y);
            sb.Draw(LineBatch._empty_texture, r, maskColor);
        }


        // ----------------------- text alignment routine untested ------------------------------------------------------------------------------------
        [Flags]
        public enum Alignment { Center = 0, Left = 1, Right = 2, Top = 4, Bottom = 8 }

        public static void DrawString(SpriteBatch sb, SpriteFont font, string text, Rectangle bounds, Alignment align, Color color)
        {
 ...
 // ------------------------------- Polygon12 Class ------------------------------------------------------------------------
    ...
    public class Polygon12
    {
        ...
        /// <summary>
        /// This returns an axis aligned bounding box based on the  corners of the polygon.
        /// The points should be a convex polygon, but this routine will work in all cases
        /// (note it can probably be done faster using the Max and Min functions but it deliberately this way so students can understand it)
        /// </summary>
        public Rectangle getAABoundingRect()
