...
namespace RC_Framework
{
    ...
    /// </summary>
    public class SpriteList : RC_Renderable, IEnumerator, IEnumerable
    {
        Sprite3[] sprite;
        ...
        /// <summary>
        /// For Ienumerator current position
        /// </summary>
        int position = 0;
...
/// <returns></returns>
        ...
        public bool MoveNext() // for IEnumerator
        {
            ...
            {
                position++;
                if (position >= sprite.Length)
                {
                    position = -1;
                    return false;
                }
            } while (sprite[position] == null || !sprite[position].active);
            ...
            for (int i = 0; i < highUsed + 1; i++)
            {
                if (sprite[i] != null && sprite[i].active)
                    if (sprite[i].getVisible())
                        sprite[i].animationTick(gameTime);
            }
        }
        ...
        /// <summary>
        /// Draw all active and visible sprites Bounding sphere 
        /// </summary>
        /// <param name="sb"></param>
        public void drawAVBoundingSphere(SpriteBatch sb, Color colorBS)
        {
            for (int i = 0; i < highUsed + 1; i++)
            {
                if (sprite[i] != null)
                    if (sprite[i].getVisible() && sprite[i].active)
                        sprite[i].drawBoundingSphere(sb, colorBS);
            }
        }
        ...
        /// return true only if all functions return true (or possibly there were no active sprites)
        /// the function has the formal parameters bool name(Sprite3 s)
        /// </summary>
