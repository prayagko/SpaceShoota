...
//  ---------------------- Change Log ----------------------
//
// Last changed 26/2/2020 to add animationHasRun 
//
//  ---------------------- End ----------------------
...
namespace RC_Framework
{
    ...
    public class Sprite3Parent : RC_RenderableBounded
    {
        ...
        /// <summary>
        /// For user controll set to true if has been visible on screen (for things that start off screen)
        /// </summary>
    ...
    */
    ...
    public class Sprite3 : Sprite3Parent
    {
        ...
        /// </summary>
        public const string SpriteVersion ="V3.18.3";
        ...
        /// there is also tagint and tag from renderable - they are sort of duplicates 
        /// </summary>
        public int varInt0;
        ...
        // ------------- User available variables above -------

        /// <summary>
        /// this variable called hitPoints is available for use by user code
        /// it is also used by the attached renderable HealthBar if one is active
        /// </summary>
        ...
        /// <summary>
        /// The sprites texture2d for basic sprites - do not change it arbitarily
        /// use setTexture(Texture2D tw) routine instead to preserve frame information
        /// </summary>
        public Texture2D texBase;

        /// <summary>
        /// The sprites texture2d for sprites using a FrameSource usually in all other cases it is identical to texBase
        /// do not change it arbitarily
        /// use setTexture(Texture2D tw) routine instead to preserve frame information
        /// it is updated frame by frame when frameSource is used 
        /// </summary>
        protected Texture2D texFrame;
        ...
        /// <summary>
        /// Animation system can have various behaviours 
        /// the possible behaviours are: 
        ///  * the whole texture the sprite is all of it
        ///    to do this set xframes = 1 and yframes = 1 (default behaviour)
        ///    
        /// 
        ///  * simple animation system (x and y simple frame animation system, ajacent frames in a 2d grid)
        ///    to do this set Xframes > 1 or  Yframes > 1 
        ///    
        /// 
        ///  * the texture but take a part of it 
        ///    to do this set image Source from imageSource and set xframes = 0 (part of the texture)
        ///
        ///  
        ///  * complex frame aniumations  
        ///    to do this set if frameSource != null (set it to a frame source)
        ///        then you get the new behaviour     
        ///        * the new behaviour uses RC_Frame class and specifically a FrameSource
        ///        * by constructing a frame source you can have maximum control of the aniamtion
        ///        * frame sources update the frames in Update this will be called when you call animationTick on the sprite
        /// </summary>
        ...
        /// <summary>
        /// This is set to true when the animation has run one complete loop
        /// </summary>
        public bool animationHasRun = false;
        ...
        /// </summary>
        public int firstFrame; // first anim frame

        /// <summary>
        /// the last animation frame withing sequence 'animationSeq'
        /// </summary>
        public int lastFrame;  // last anim frame
        ...
        /// it also does the same for shrink and grow (well shrink actually)
        /// and fade as well 
        /// </summary>
        internal bool makeInactive = false;
        
        /// <summary>
        /// set to true to activate the sprite fade routines in tick()
        /// use setFadeDetails(bool fadeQ, Color fadeColourQ, int fadeTicks);
        /// </summary>
        private bool fade = false; // dont vary colour in tick()
        
        /// <summary>
        /// color to fade from - see fade
        /// </summary>
        private Color fadeColorStart = new Color(255, 255, 255, 255); // white
        
        /// <summary>
        /// color to fade to 
        /// </summary>
        private Color fadeColorEnd = new Color(255, 255, 255, 0); // transparent white
        
        /// <summary>
        /// time in ticks to fade colour
        /// </summary>
        private int fadeTicks = 15; // sensible default  

        /// <summary>
        /// Internal counter to control fading
        /// </summary>
        private int ticksFaded =0;
        ...
        /// also it can be moved to boundingPoly for a non AA bounding box using Util2.Polygon12(Rect4 r) constructor
        /// </summary>
        ...
        private WayPointList wayListVar = null;

        /// <summary>
        /// The waypoints for the waypoint move routines
        /// at this time you need a separate waypointlist for each sprite so setting this uses a copy constructor
        /// </summary>
        public WayPointList wayList
        {
            set
            {
                wayListVar = new WayPointList(value);
            }
            get
            {
                return wayListVar;
...
// ------------------------------------------------ Constructors ----------------------------------------------------------------------
        ...
        /// <summary>
        /// Constructor with a image source rectangle
        /// </summary>
        /// <param name="visibleZ"></param>
        /// <param name="texZ"></param>
        /// <param name="x"></param>
        /// <param name="y"></param>
        public Sprite3(bool visibleZ, Texture2D texZ, Rectangle imageSrc, float x, float y)
        {
            initVars();
            visible = visibleZ;
            setTexture(texZ, true);
...
// ------------------------------------------------ End Constructors ----------------------------------------------------------------------
        ...
        {
            if (texBase == null || computeNewSize) // previously null or new size
            {
                texBase = tw;
                texFrame = tw;
        ...
        /// </summary>
        public Texture2D getTextureBase()
        {
            return texBase;
        }
        ...
        /// <summary>
        /// Moves the sprite by the delta ds.x and delta ds.y 
        ...
        /// Moves the sprite by the delta ds.x and delta ds.y 
        /// uses an input delta so no need to call setDeltaSpeed(Vector2 ds)
        /// This is used to add a second x,y movement so its does not disturb old positions and hence will
        /// preserve move angle
        /// </summary>
        public void addMoveByDeltaXY(Vector2 ds)
        {
            //oldPos.X = pos.X;
            //oldPos.Y = pos.Y;
            pos.X = pos.X + ds.X;
        ...
        /// <summary>
        /// Creates a move vector2 in angle speed so addMoveByDeltaXY
        /// can add in an angle/speed movement
        /// </summary>
        public Vector2 computeMoveByAngleSpeed()
        {
            //oldPos.X = pos.X;
            //oldPos.Y = pos.Y;
 ...
 /// <returns>true if we got there</returns>
        ...
        {
            if (texBase != null) setWidthHeightOfTex(texBase.Width,texBase.Height); // heigth of the useful psrt of the texture
        }
        ...
        public Rectangle getActualImageSource()
        {
            ...
            {
                // use frame source
                RC_Frame f = frameSource.getFrame();
                texFrame = f.tex;
                return f.source;
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        // use a animation frame 
                        Rectangle r = new Rectangle();
                        r.Width = XframeWidth;
                        r.Height = YframeHeight;
                        r.X = Xframe * r.Width;
                        r.Y = Yframe * r.Height;
                        return r;
                    }
        ...
        public void draw(SpriteBatch sb)
        {
            ...
            {
                prepareToDraw();
                sb.Draw(texFrame, dest,src, getColor(), displayAngle+displayAngleOffset, rotateImage, flip, 0.5f);
        ...
        public void setBBToTexture()
        {
            bb.X = 0;
            bb.Y = 0;
            bb.Width = texBase.Width; // Bounding Box width 
            bb.Height = texBase.Height; // Bounding Box height 
        }
        ...
        {
            bb.Width = Convert.ToInt32(texBase.Width * f); // Bounding Box width 
            bb.Height = Convert.ToInt32(texBase.Height * f); // Bounding Box height           
            bb.X = Convert.ToInt32(texBase.Width * (1 - f) / 2);
            bb.Y = Convert.ToInt32(texBase.Height * (1 - f) / 2);
        }
        ...
        public void setBBandHSFractionOfTexCentered(float f)
        {
            bb.Width = Convert.ToInt32(texBase.Width * f); // Bounding Box width 
            bb.Height = Convert.ToInt32(texBase.Height * f); // Bounding Box height           
            bb.X = (int)Math.Round(texBase.Width * (1 - f)/2.0); // 
            bb.Y = (int)Math.Round(texBase.Height * (1 - f)/2.0); // 
            hotSpotOffset.X = texBase.Width / 2;
            hotSpotOffset.Y = texBase.Height / 2;
        }
        ...
        /// <summary>
        /// Draw the destination square (for debugging only)
        /// valid after call to draw only
        /// </summary>
        /// <param name="sb"></param>
        /// <param name="c"></param>
        public void drawDest(SpriteBatch sb, Color c)
        {
            LineBatch.drawLineRectangle(sb, dest, c);
        }
        ...
        /// this routine now has gameTime as a parameter
        /// </summary>
        public void animationTick(GameTime gameTime)
        {
            if (frameSource != null) frameSource.Update(gameTime);
        ...
        public void setFadeDetails(bool fadeQ, Color fadeFromColourQ, Color fadeToColourQ, int fadeTicksQ, bool makeInactiveQ)
        {
        makeInactive = makeInactiveQ; 
        fade = fadeQ; 
        fadeColorStart = fadeFromColourQ;
        fadeColorEnd = fadeToColourQ;
    ...
    // ************************************** class SpriteFactoryParent ********************************************************** //

    /// <summary>
    /// This is a parent class for classes that create sprites - generators and such
    /// </summary>
    public abstract class SpriteFactoryParent : RC_Renderable // from renderable to give us update loops and ability to use lists 
    {
        /// <summary>
        ...
        {
            return generateAt(kind, param, new Rectangle(0, 0, 0, 0));
        }
        ...
        /// <param name="param"></param>
        /// <returns></returns>
        public virtual Sprite3 generate(Rectangle boundz)
        {
            return generateAt(0, 0, new Rectangle(boundz.X, boundz.Y, boundz.Width, boundz.Height));
        }
        ...
        /// <returns></returns>
        public abstract Sprite3 generateAt(int kind, int param, Rectangle boundz);
    ...
    public class SimpleSpriteFactory : SpriteFactoryParent
    {
        Texture2D tex;
        float width;
        float height;
        RC_PositionFactory posFactory;

        public SimpleSpriteFactory(Texture2D texQ, RC_PositionFactory posFactoryQ, float widthQ, float heightQ)
        {
            tex = texQ;
            posFactory = posFactoryQ;
            width = widthQ;
            height = heightQ;
        }

        public override Sprite3 generateAt(int kind, int param, Rectangle boundz) // note it ignores all parameters
        {
            Vector2 poss = posFactory.getNextPos();
            Sprite3 retv = new Sprite3(true, tex, poss.X, poss.Y);
            retv.setBBToTexture();
            return retv;
        }
    }
