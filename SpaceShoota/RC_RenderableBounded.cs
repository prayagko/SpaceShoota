...
namespace RC_Framework
{
    ...
    /// <summary>
    /// Would be usefull a comment here Annekes Button built out of two textures
    /// </summary>
    public class ButtonAnn : RC_RenderableBounded
    {
        //public string text { get; set; }
        public SpriteFont font { get; set; }
        public Vector2 textOffset;
        ...
        public ButtonAnn(Texture2D backGroundTex, string textZ, SpriteFont fontZ, Vector2 pos, Color fontColorZ, Color backColor)
        {
            active = true;
            visible = true;
            text = textZ;
...
// ******************************** grid background ******************************************************
    ...
    public class TextureFlash : RC_RenderableBounded
    {
        ...
        public override void Draw(SpriteBatch sb)
        {
            if (!isTransparent  && visible) sb.Draw(tex, bounds, source, colour);
        }
        ...
        // ************************************** Mask   like a letterbox  ***********************************************

        public class LetterboxMask : RC_RenderableBounded
        {
            Rectangle outerRectangle;
            int widthOfMask;
            Color maskColor;

            public LetterboxMask(Rectangle outerRectangleQ, int widthOfMaskQ, Color maskColorQ)
            {
            outerRectangle = outerRectangleQ;
            widthOfMask = widthOfMaskQ;
            maskColor = maskColorQ;
            }
